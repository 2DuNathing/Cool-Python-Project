...
class SegHandler:
    ...
    def scale_from_global_angle(self, x, rw_normal_angle, rw_distance):
        ...
        num = SCREEN_DIST * math.cos(math.randians(rw_normal_angle - x.angle - self.player.angle))
        den = rw_distance * math.cos(math.randians(x_angle))
    ...
    def init_screen_range(self):
        self.screen_range = set(range(WITH))

    def draw_solid_wall_range(self, x1, x2):
        #some aliases tp shorten the following code
        seg = self.seg
